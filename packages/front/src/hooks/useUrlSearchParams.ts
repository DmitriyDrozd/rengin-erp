...
export type ParamsWithTemplate<T> = Partial<{
...
export type UseUrlSearchParamsResults<TT> = [Partial<ParamsWithTemplate<TT>>, (nextQuery: InitialType) => void];


const SUPPORTED_PARAMS_TYPES = [Number, String, Boolean, Date];

function setQueryToCurrentUrl<T>(params: Partial<ParamsWithTemplate<T>>): URL {
    const { URL } = getWindow();
...
export function useUrlSearchParams<TT extends {}>(

    initial: Partial<ParamsWithTemplate<TT>> = {},
    types: TT ,
    replace = false
): UseUrlSearchParamsResults<TT> {
    ...
    const params: ParamsWithTemplate<TT> = useMemo< Partial<ParamsWithTemplate<TT>>>(() => {
        let result = [];
    ...
    const setParams = (params:  Partial<ParamsWithTemplate<TT>>) => {
