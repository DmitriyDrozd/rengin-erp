...
export const ExpenseEstimationsStatusCellEditor = forwardRef((props: CellEditorProps<typeof EXPENSES, string>, ref) => {
    ...
    return (
        <Select
            options={optionsFromValuesList(EXPENSES.properties.estimationsStatus.enum).map(o => {
                if (role === 'менеджер') {
                    const availableStatuses = estimationsStatusesRulesForManager[expense.estimationsStatus];
