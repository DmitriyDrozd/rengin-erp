...
import EditIssueItemForm from './tabs/EditTicketFormTab'
...
export default () => {
    const {entity,id,item,verb} = usePathnameResource()
    useUnmount(() => {})
    ...
    const title = entity.getItemName(ticketState)
    const onSave = () => {
        //const { expenses,expensePrice,estimationPrice,estimations, ...rest} = ticketState
        dispatch(entity.actions?.patched(ticketState))
        onBack()
    }
