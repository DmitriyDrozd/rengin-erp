...
export default async (doc: GoogleSpreadsheet, store: FrontStore) => {
    ...
    const addTable = async  <EID extends string, Attrs extends AnyAttributes>(entity: EntitySlice<Attrs,EID>)  => {
        const title = entity.langRU.plural
        let sheet = doc.sheetsByTitle[title]
        const headerProps = getResourceHeaderProps(entity)
        ...
        const items = entity.selectors.selectAll(store.getState())
...
const getResourceHeaderProps =
    <EID extends string, Attrs extends AnyAttributes>(entity: EntitySlice<Attrs,EID>) =>
        entity.attributesList
