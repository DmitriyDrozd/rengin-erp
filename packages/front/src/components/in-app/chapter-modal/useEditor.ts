...
> ( editor: RenEditor<EID, Attrs>,
    idOrInitItem: 'create' | string | ItemByAttrs<Attrs,EID>
   ) => {
    ...
    const [newItem] = useState({id: generateGuid(), ...getArrayPropsForNewItem(entity.attributes)})
    const state = useORMState()
    const items: Item[] = useSelector(editor.entity.selectors.selectAll) as any
    ...
    const result = {
        ...
        remove: () => {
            dispatch(editor.entity.actions.removed(id))
        },
        save: () => {
            if(mode === 'create')
                dispatch(editor.entity.actions?.added(item))
           else  if(JSON.stringify(initItem) !== JSON.stringify(item))
            dispatch(editor.entity.actions?.patched(item))
        },
        updateItemProperty,
        validateProps: () => {
        ...
        getRenFieldProps:<K extends keyof Attrs>(prop: K) => {
            return {mode: mode as 'create' |'edit',
                ...
                value: item[prop],
                property: editor.entity.attributes[prop],
                updateItemProperty: updateItemProperty(prop),
                error: errors[prop] as any as string | undefined,
                params: params[prop] as any as GetParamsByMeta<typeof entity.attributes[K]>
            }
        }
    }
...
export const useContextEditorProperty =  <EID extends string, Attrs extends AnyAttributes, K extends keyof ItemByAttrs<Attrs,EID>>
(entity: EntitySlice<Attrs, EID> | string, property?: K) => {
    const editor = useContextEditor<EID,Attrs>()
    return editor.getRenFieldProps(property || entity)
}
