...
interface SessionOptions {
	...
	/**
	 * Whether to trust the last event ID given by the connection in the `Last-Event-ID` request header.
	 *
	...
	/**
	 * Time in milliseconds for the connection to wait before attempting to reconnect if the connection is closed.
	 *
	 * This is a request to the connection browser, and does not guarantee that the connection will actually respect the given time.
	 *
	 * Equivalent to immediately calling `.retry().dispatch().flush()` after a connection is made.
	 *
	 * Give as `null` to avoid sending an explicit reconnection time and allow the connection browser to decide itself.
	 *
	...
	/**
	 * Status code to be sent to the connection.
	 *
	 ...
	 *
	 * A connection can be asked to stop reconnecting by using 204 status code.
	 *
	 * Defaults to `200`.
	 */
	statusCode?: number;
...
interface StreamOptions {
	/**
	 * Event name/type to be emitted when stream data is sent to the connection.
	 *
	 * Defaults to `"stream"`.
	 */
	eventName?: string;
...
interface IterateOptions {
	/**
	 * Event name/type to be emitted when iterable data is sent to the connection.
	 *
...
/**
 * A Session represents an open connection between the server and a connection.
 *
 * It extends from the {@link https://nodejs.org/api/events.html#events_class_eventemitter | EventEmitter} class.
 *
 * It emits the `connected` event after it has connected and flushed all headers to the connection, and the
 * `disconnected` event after connection connection has been closed.
 *
...
> extends TypedEmitter<SessionEvents> {
	/**
	 * The last ID sent to the connection.
	 *
	 ...
	 *
	 * Note that this does **not** send the written data to the connection. Use `flush` to flush the internal buffer.
	 */
	dispatch = (): this => {
		this.buffer += "\n";
	...
	/**
	 * Flush the buffered data to the connection and clear the buffer.
	 */
	flush = (): this => {
	...
	/**
	 * Pipe readable stream data to the connection.
	 *
	 * Each data emission by the stream emits a new event that is dispatched to the connection.
	 ...
	 * @param options - Options to alter how the stream is flushed to the connection.
	 *
	...
	/**
	 * Iterate over an iterable and send yielded values as data to the connection.
	 *
	 * Each yield emits a new event that is dispatched to the connection.
	 * This uses the `push` method under the hood.
	 *
