...
export const importBackup = ({ file, folderName }: { file?: any, folderName?: string }): number => {
    ...
    const restoreBackupCommand: string = `docker exec mongodb sh -c "mongorestore -h localhost:27017 -d rengin -u rengin -p BuildMeUp --authenticationDatabase admin targetdb ${src}"`;
